...
## Equality.
@add_predicate(sequence_qc, "Equality Reflexive", seq::Seq -> seq == seq)
...
## Bitwise operations.
@add_predicate(sequence_qc, "Negation Involutive", seq::Seq -> ~~seq == seq)
...
@add_predicate(
    sequence_qc,
    "& absorbing element",
    seq::Seq -> seq & Sequence(falses(length(seq))) == Sequence(falses(length(seq)))
)
...
@add_predicate(
    sequence_qc,
    "| absorbing element",
    seq::Seq -> seq | Sequence(trues(length(seq))) == Sequence(trues(length(seq)))
)
