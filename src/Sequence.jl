...
"""
    ...
    $(FUNCTIONNAME)(mask, sample, h, ω; wipe = true)
...
when applied to `h.data.chunks`. The second forms acts similarly for an
interval (or set of intervals). If `x` is a vertex, the third form returns a
mask that `&` the markers of the associated haplotype not ancestral for it to 0.
The ancestral intervals of `x` is defined as the union of its downstream edges'
intervals. Behaviour is similar if `x` is an edge, but the mask is meant for the
intersection of chunks between its adjacent vertices.
...
function ancestral_mask!(mask, h::Sequence, idx; wipe = true)
    bs = blocksize(h)
    nchunks = length(h.data.chunks)

    wipe && wipe!(mask)

    chunks_idx = chunkidx(h, idx.start), chunkidx(h, idx.stop)
    ...
    ## First chunk
    ormask = typemax(UInt)
    let nleading0 = idxinchunk(h, idx.start) - 1,
