...
function EIterMGraph(EIMG, genealogy, ωs, stack::CheapStack, visited,
    bp_pars, root)
    fill!(visited, false)

    ret = EIMG(genealogy, ωs, stack, visited, bp_pars)
...
EIterMGraph(EIMG, genealogy, ωs, store::AbstractArray, visited, bp_pars,
    root) =
    EIterMGraph(EIMG, genealogy, ωs, CheapStack(store), visited, bp_pars, root)

block_predicate(::Any, e) = true

function iterate(iter::AbstractEIterMGraph, state = 1)
    stack = iter.stack
...
--*Internal*--
"""
struct EdgesInterval{T, I, E} <: AbstractEIterMGraph
...
EdgesInterval(genealogy, ωs, stack, visited, min_latitude, root) =
    EIterMGraph(EdgesInterval, genealogy, ωs, stack, visited, min_latitude, root)
