...
`r + k`.
"""
function _impedance_update_C!(C, arg, ss, ds, edgesmap, estack, vqueue, visited)
    r = nrecombinations(arg)
    k = 1

    ## Sources cycles ##
    s1, iter_s = Iterators.peel(ss)
    for v ∈ iter_s
        vec = @view C[:, r + k]
        fill!(vec, 0)

        _path_bfs_forward!(estack, arg, s1, v, vqueue, visited)
        _bfs_backtrack!(vec, edgesmap, estack, Threads.ReentrantLock())
        k += 1
    end

    ## Destination cycles ##
    d1, iter_d = Iterators.peel(ds)
    for v ∈ iter_d
        vec = @view C[:, r + k]
        fill!(vec, 0)

        _path_bfs_forward!(estack, arg, d1, v, vqueue, visited)
        _bfs_backtrack!(vec, edgesmap, estack, Threads.ReentrantLock())
        k += 1
    end
    ...
    _path_bfs_forward!(estack, arg, s1, d1, vqueue, visited)
    _bfs_backtrack!(vec, edgesmap, estack, Threads.ReentrantLock())
