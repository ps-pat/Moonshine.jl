...
function ancestral_mask!(mask, sample::Sample, h, ω::Ω; wipe = true)
    wipe && wipe!(mask)

    idx = postoidx(sample, ω)
    (iszero ∘ length)(idx) && return mask

    ancestral_mask!(mask, h, idx, wipe = false)
end

ancestral_mask(sample::Sample, h, ω::Ω) =
    ancestral_mask!(similar(h.data.chunks), sample, h, ω)

function ancestral_mask!(mask, sample::Sample, h, ωs; wipe = true)
    wipe && wipe!(mask)

    for ω ∈ ωs
        ancestral_mask!(mask, sample, h, ω, wipe = false)
    end

    mask
end

ancestral_mask(sample::Sample, h, ωs) =
    ancestral_mask!(similar(h.data.chunks), sample, h, ωs)
